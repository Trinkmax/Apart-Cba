              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-sm truncate group-hover:text-primary transition-colors">{g.full_name}</span>
                    {g.blacklisted && (
                  ...
                  <div className="flex items-center gap-3 mt-0.5 text-xs text-muted-foreground">
                    ...
                    {g.email && <span className="flex items-center gap-1 truncate"><Mail size={10} /> {g.email}</span>}
                    {g.country && g.country !== "AR" && (
                      <span className="flex items-center gap-1"><Globe size={10} /> {g.country}</span>
                    )}
                  </div>
                </div>
                <div className="text-right text-xs">
                  ...
                  {g.last_stay_at && (
                    <div className="text-muted-foreground mt-0.5">{formatTimeAgo(g.last_stay_at)}</div>
                  )}
                </div>
              </Link>
            ))}
